...
const HEADER_ALIASES = {
  ...
  duration: ["time limit", "time limit (sec)", "duration"],
  category: ["category"],
...
const GAME_FIELD_ALIASES = {
  ...
  default_question_duration_seconds: ["question duration"],
...
export async function parseQuestionsWorkbook(buffer) {
  ...
  const idx = {
    ...
    duration: findHeaderIndex(headerRow, HEADER_ALIASES.duration),
  ...
  for (let r = 2; r <= sheet.rowCount; r++) {
    ...
    questions.push({
      ...
      duration_seconds: idx.duration !== -1 ? toInt(cellText(values[idx.duration]), null) : null,
